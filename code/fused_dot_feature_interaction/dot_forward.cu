...
__global__ void DotFeatureInteractionTensorCore(
    int m_num_tiles, int k_num_tiles, int64_t batch_size, int padded_num_rows,
    int vector_num_pack, int padded_vector_num_pack, int out_num_cols,
    int out_num_cols_num_pack, int in_shared_mem_cols,
    int in_shared_mem_cols_num_pack, int acc_shared_mem_cols,
    int acc_shared_mem_cols_num_pack, int offset, int output_padding,
  ...
  Pack<T, pack_size> *buf_pack =
      reinterpret_cast<Pack<T, pack_size> *>(shared_buf);
  ComputeType *acc_buf = reinterpret_cast<ComputeType *>(
      shared_buf + padded_num_rows * in_shared_mem_cols * sizeof(T));
...
#pragma unroll
  ...
  __syncthreads();
  for (int blocks_id = warp_id; blocks_id < m_num_tiles * m_num_tiles;
    ...
    if (blocks_row_id >= blocks_col_id) {
      ...
      nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, tile_dim, tile_dim,
                             k_tile_dim, PrecisionType, nvcuda::wmma::col_major>
          b;
      nvcuda::wmma::fill_fragment(acc, 0.0f);
      for (int step = 0; step < k_num_tiles; ++step) {
        T *tile_a_ptr = buf + blocks_row_id * tile_dim * in_shared_mem_cols +
                        step * k_tile_dim;
        T *tile_b_ptr = buf + blocks_col_id * tile_dim * in_shared_mem_cols +
                        step * k_tile_dim;
    ...
    for (int k = 0; k < unroll_dim; ++k) {
      ...
      for (int col = threadIdx.x; col < param.features_dim; col += blockDim.x) {
        if (col < row + offset) {
          ...
          emb_out[idx] =
              static_cast<T>(acc_buf[row * acc_shared_mem_cols + col]);
        }
      }
    }
  }
...
int main() {
  ...
  DotFeatureInteractionTensorCore<T, ComputeType, 2, 4, TILE_DIM, K_TILE_DIM>
  ...
  std::ifstream out_is;
  out_is.open("out.bin");
  std::vector<half> out_data(batch_size * out_num_cols);
