...
__device__ void
cuApplyLayerNorm_(V *__restrict__ output_vals, U *__restrict__ mean,
...
void HostApplyLayerNorm(V *output, U *mean, U *invvar, const T *input, int n1,
  ...
  auto stream = nullptr; // at::cuda::getCurrentCUDAStream().stream();
...
void HostLayerNormGradient(const V *dout, const U *mean, const U *invvar,
  ...
  if (gamma != NULL && beta != NULL) {
    ...
    U *part_grad_gamma_ptr;
    U *part_grad_beta_ptr;
    cudaMalloc(&part_grad_gamma_ptr, part_size * n2 * sizeof(U));
    cudaMalloc(&part_grad_beta_ptr, part_size * n2 * sizeof(U));
    cuComputePartGradGammaBeta<<<blocks2, threads2, nshared2, stream>>>(
        dout, input, n1, n2, mean, invvar, U(epsilon), part_grad_gamma_ptr,
        part_grad_beta_ptr);
    ...
    cuComputeGradGammaBeta<<<blocks3, threads3, nshared3, stream>>>(
        part_grad_gamma_ptr, part_grad_beta_ptr, part_size, n1, n2, grad_gamma,
        grad_beta);
  }
