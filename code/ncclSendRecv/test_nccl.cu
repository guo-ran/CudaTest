...
int main() {
  ...
  for (int gpu_id = 0; gpu_id < n_gpu; gpu_id++) {
    ...
    Check(ncclGroupStart());
    for (int64_t i = 0; i < parallel_num; ++i) {
      Check(ncclSend(send_datas.at(i) + send_offsets.at(i), send_elem_cnt.at(i), ncclDataType_t::ncclFloat16, i,
                             comms.at(i), streams.at(i)));
      Check(ncclRecv(recv_datas.at(i) + recv_offsets.at(i), recv_elem_cnt.at(i), ncclDataType_t::ncclFloat16, i,
                             comms.at(i), streams.at(i)));
    }
    //for (int64_t k = 0; k < parallel_num; ++k) {
    //  //int i = (gpu_id + k + 1) % parallel_num;
